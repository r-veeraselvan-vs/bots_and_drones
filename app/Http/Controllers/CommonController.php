...
class CommonController extends Controller
{
    ...
    public function index()
    {
        ...
        $supporting_partner = SupportingPartner::where('status', 'Active')->orderBy('id', 'desc')->get();
    ...
    public function products(Request $request, $menu, $slug = 'all', $sub_slug="all")
    {
        ...
        if($request->type_certified!=null)
        {
            ...
            {
                $p = $p->where('type_certified', $request->type_certified);
            }
            else
            {
                $p = $p;
        ...
        if($request->country_ID!=null)
        {
           Session::put('country_ID',$request->country_ID); 
            if($request->country_ID!="All")
                {
                    $p = $p->where('country',$request->country_ID);
                }
                else
                {
                    $p = $p;
        ...
        if($request->min_price!=null && $request->max_price!=null )
        {
            ...
            $p =  $p->whereRaw('REPLACE(`usd_price`, ",","")')->whereBetween('usd_price', [ $request->min_price,  $request->max_price]);
    ...
    public function resetFilter()
    {
        ...
        $keysToForget = [
            ...
            'max_price', 'item_type', 'sort', 'country_ID',
        ];
