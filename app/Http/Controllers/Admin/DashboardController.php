...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        foreach ($userContactCounts as $userContactCount) {
            ...
            if ($user) {
                ...
                $user->products = Products::select('products.id as product_id', 'products.title as product_name', 'products.usd_price')
                    ->leftJoin('contacts', 'products.id', '=', 'contacts.product_id')
                    ->selectRaw('products.id as product_id, products.title as product_name, products.usd_price, COUNT(contacts.id) as contact_count')
                    ->where('contacts.seller_id', $user->id)
                    ->groupBy('products.id', 'products.title', 'products.usd_price')
                ...
                // For now, let's just skip this user
                continue;
            }
        }
        ...
        $topLocations = DB::table('contacts')
                    ...
                    ->join('country', 'products.country', '=', 'country.id')
    ...
    public function buyersList(Request $request)
    {
        ...
        if ($country) {
            $buyersQuery->where('country_id', $country);
        }

        $buyers = $buyersQuery->paginate(10);

        return view('admin.buyers', compact('buyers', 'startDate', 'endDate', 'country'));
    }
    ...
    public function sellersList(Request $request)
    {
        ...
        if ($country) {
            $sellersQuery->where('country_id', $country);
        }

        $sellers = $sellersQuery->paginate(10);

        return view('admin.sellers',compact('sellers', 'startDate', 'endDate', 'country'));
    }
    ...
    public function enquiryList(Request $request)
    {
        ...
        // Assuming you have a relationship set up between Contact and Product
        $enquiriesQuery->with('product');

        if ($country) {
            // Filter enquiries based on product country
            $enquiriesQuery->whereHas('product', function ($query) use ($country) {
        ...
        return view('admin.enquiries', compact('enquiries', 'startDate', 'endDate', 'country'));
    }
