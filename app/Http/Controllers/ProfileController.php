...
class ProfileController extends Controller
{
    public function index() {
        ...
        {
            $user = Auth::user();
            $country = Countries::where('id', $user->country_id)->first('name');

            if ($user) {
                return view('profile.index', compact('user', 'country'));
            }
        }
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $new->trading_name = $request->trading_name;
        $new->website_link = $request->website_link;
