...
@endif
     ...
     @if (Auth::user()->seller == 'Y')
        @php
            $enquiryCount = \App\Models\Contact::where('seller_id', Auth::id())->where('status', 'Y')->count();
        @endphp

        <li class="nav-item {{ (request()->segment(1) == 'drone_enquiries') ? 'active' : '' }}">
            <a class="nav-link " href="{{ route('enquiry.list') }}">
                <i class="bi bi-file-arrow-down"></i>
                <span>Buyer Enquiries</span>&nbsp
                <span class="badge badge-pill badge-info">{{ $enquiryCount }}</span>
            </a>
        </li><!-- End Dashboard Nav -->
    @endif
