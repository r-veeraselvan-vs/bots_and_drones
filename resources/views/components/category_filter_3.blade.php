  <?php 
            ...
            $array = explode('?', $url);
             ...
             <div class="filter-block">
                ...
                >
                  ...
                  <?php 
                   $locations = App\Models\Products::select('state')->groupBy('state')->where('status', 'Y')->where('state','!=',null)->orderby('state','asc')->where('category_id','2')->get();
            ...
            <!-- Start of product filter -->
                ...
                <a
                  class="filter-head"
                  data-bs-toggle="collapse"
                  href="#filterCollapse-equipmentType"
                  role="button"
                  aria-expanded="true"
                  aria-controls="filterCollapse-equipmentType"
                >
                ...
                <div
                  class="filter-collapse-container collapse show"
                  id="filterCollapse-equipmentType"
                >
                   <div class="form-sec">
                <select name="equipmentType" id="equipmentType" onchange="filter()" class="form-select">
                   ...
                   <?php 
                                            ...
                                            ?>
                                             @foreach($types as $type)
                                                 <option value="{{$type->name}}"  @if($type->name  == request()->equipment_item_type) selected @endif>{{$type->name}}</option>
                                              @endforeach
              ...
              <div class="filter-block">
                <a
                  class="filter-head"
                  data-bs-toggle="collapse"
                  href="#filterCollapse-usedFor"
                  role="button"
                  aria-expanded="true"
                  aria-controls="filterCollapse-usedFor"
                >
              ...
              <div class="filter-block">
                ...
                <select name="item_type" id="brand"  onchange="filter()" class="form-select">
                  <option value="">Select Brand</option>
                   <?php 
                    $brands = App\Models\Products::select('brand')->distinct()->where('category_id','=',2)->where('status','Y')->orderby('brand','asc')->get();
                   ?>
                   <option value="All">All</option>
